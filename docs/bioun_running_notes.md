...
## NPO/RMU are currently blocked by the shared-GPU contention -- not fixable via batch size or sequence length

With the other tenant's job holding ~8.3GB, NPO OOM'd identically (`torch.OutOfMemoryError`, exactly 30.88 GiB in use, 19.75 MiB free, tried to allocate 20 MiB) at both `per_device_train_batch_size=1` and after reducing dataset `max_length` 512->288. The identical 30.88 GiB figure across both attempts confirms sequence length / activation memory is not the bottleneck -- the ~30GB is essentially fixed cost from the trainable model + the frozen reference-model deep-copy (`grad_diff.py::_prepare_ref_model`) + optimizer state, none of which shrink from a shorter sequence or smaller batch. There are two real paths forward, neither attempted yet:
1. Wait for the other tenant's job to finish and reclaim its ~8.3GB (the arithmetic suggests this alone would be enough: 30.88 + 8.3 = 39.18 GiB, just under the 39.39 GiB card capacity).
2. Modify `_prepare_ref_model` to load the reference copy in 8-bit (bitsandbytes) instead of a full-precision `copy.deepcopy` -- would roughly halve its footprint, but is a change to shared framework code used by every KL-based method (GradDiff-KL, NPO, RMU, UNDIAL, WGA, SatImp, DPO), so it needs deliberate testing before trusting its outputs, not just a quick patch.
