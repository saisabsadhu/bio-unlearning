...
**Honest interpretation**: concept-specific eval trades dilution-driven signal loss (whole benchmark) for floor/ceiling-driven noise (n=4-5 per concept) -- it relocates the statistical-power problem rather than solving it. The actual fix is generating substantially more concept-specific instances (15-20+ per concept) via the same real-citation pipeline already used for the RGU expansion, not re-slicing the existing tiny per-concept sets differently.

### 8.12 Scaling concept-specific instances to n=16-19 resolves it -- two new significant own-scenario findings

Generated 15 more instances per concept (`stage_b_instances/generate_concept_specific_instances.py`), each grounded in the SAME real reversal event already documented in that concept's existing instances (same FDA/USPSTF/WHI/Merck source, only the clinical angle varied) -- 52/60 passed quality filtering (87%), bringing each concept to 16-19 total instances. Re-ran the identical n=3-vs-n=3 comparison (`data/gate2_results/OGDA_concept_specific_n3vn3_v2_statistical_comparison.json`):
...
No more degenerate NaN values (the n=4-5 floor effect is gone), and two real, new findings emerge: **HRT and Vioxx now show large, statistically significant own-scenario DEF *enhancement* under real OGDA relative to random-subspace control** (z=3.44, z=5.22) -- real OGDA achieves meaningfully better own-scenario forget-and-retain behavior than a random edit of the same size, a genuine positive own-scenario result not previously established this clearly for either concept. Aspirin's FA result is now directionally *consistent* with the whole-benchmark finding (z=2.0, positive, same direction as the whole-benchmark z=4.47) rather than reversed as it was at n=4-5 -- weaker, but no longer contradictory. Rosiglitazone shows no significant signal in either metric at this sample size.

**Honest calibration**: n=16-19 is still a modest sample, not a large one -- these results should be read as real findings at an honestly-scoped confidence level, not as beyond question. But the specific failure mode that made the n=4-5 result untrustworthy (floor/ceiling effects, direction-reversal vs. the whole-benchmark measure) is resolved, not just hidden by a different kind of noise. This does not affect the collateral-suppression finding (rosiglitazone/Vioxx, Section 8.9-8.10) -- that result comes from the whole-benchmark comparison on the *untargeted* scenario and doesn't depend on these own-scenario numbers. Combined, the project now has two independent, real own-scenario/collateral findings for Vioxx specifically (own-scenario DEF enhancement here, collateral DEF suppression in Section 8.10) -- worth highlighting together in the paper as the most complete single-concept picture available.

**A methodological lesson worth keeping**: debugging repeated OOM failures during this run's 16 sequential model loads revealed that much of what looked like "sustained external GPU tenant contention" throughout this session was at least partly a memory leak in the project's own long-running batch-eval scripts -- `del model; torch.cuda.empty_cache()` does not fully release CUDA memory across many sequential model loads within one Python process. Confirmed directly: killing a stuck batch process dropped GPU usage from ~40GB to ~4GB instantly, far more than the actual other tenant could account for. Fixed by isolating each remaining model load into its own OS subprocess (`stage_a_umls/ogda_concept_specific_one.py`) -- every one of the 8 remaining evaluations succeeded on the first attempt once isolated, versus repeated failures in the shared-process version. Any future long-running multi-checkpoint eval script in this project should default to subprocess-per-checkpoint, not sequential in-process loads.
