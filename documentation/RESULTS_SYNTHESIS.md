...
**The scaling-dilution finding (new, from the v2 re-eval)**: evaluating the same 4 OGDA checkpoints against the 273-example v2 RGU set instead of the 58-example pilot set, the own-scenario RGU effect washes out to noise for every concept (RGU_fa/RGU_def all within noise of baseline). This is not a retraction — each checkpoint's edit is deliberately surgical to one concept's ontology subspace, so 272 of 273 v2 examples are about unrelated topics an aspirin-specific edit was never going to move. It does mean **a concept-specific eval subset is now a precondition, not a refinement**, for OGDA's own-scenario claim to be measurable at benchmark scale at all — the top open item going forward.

**Bottom line**: the defensible current claim is narrower than originally hoped, arrived at by catching two separate n=1-vs-n=3 artifacts before they went in the paper. Aspirin's RGU FA (z=4.47) is the cleanest single result; the collateral-suppression finding on rosiglitazone/Vioxx is the more thesis-relevant one.
...
- **Concept-specific eval subset for OGDA** (§5) — now a precondition, not a refinement, per the scaling-dilution finding.
