...
**Concept-specific eval, tried directly — an honest, mixed result, not the clean fix hoped for.** Each of the 4 tested concepts already has 4-5 dedicated instances sitting in the dataset (found unused in `RGU_val`/`RGU_test` — the standard eval pipeline had always scored against the diffuse `RGU_train.jsonl`, which doesn't contain these concepts' instances at all). Built a proper n=3-vs-n=3 comparison (real-OGDA seeds 0/1/2 vs. the same random-control seeds) restricted to just those dedicated instances per concept (`data/gate2_results/OGDA_concept_specific_n3vn3_statistical_comparison.json`):

| Concept | n | FA z | DEF z |
|---|---|---|---|
| Aspirin | 5 | -1.0 | **-3.5** |
| Rosiglitazone | 5 | NaN (both real & random = 1.0, no variance) | 2.0 |
| HRT | 4 | 0.5 | NaN (both = 0.0, no variance) |
| Vioxx | 5 | 0.0 | 0.0 |

**This does not confirm the hoped-for outcome.** Two of four concepts (HRT, Vioxx) show zero differentiating signal between real-OGDA and random-control in either direction — a floor/ceiling effect from having only 4-5 examples, not evidence of anything. Aspirin does show a significant DEF result, but in the *opposite* direction from what "concept-specific eval should sharpen the signal" would predict: real-OGDA's own-scenario DEF (0.12) sits significantly *below* random-control's (0.213, z=-3.5) — real OGDA doing worse than a random edit of the same size on aspirin's own dedicated instances, not a clean positive signal. This directly reverses the whole-benchmark result for the same concept (aspirin's own-scenario RGU FA was z=4.47 on the diffuse 58-example set — clear and positive — vs. z=-1.0 here). **The two eval approaches disagree, not just in strength but in direction**, for the one concept where both produce a real comparison at all.

**Honest conclusion**: concept-specific eval does not simply fix the dilution problem — it trades dilution-driven signal loss for floor/ceiling-driven noise at n=4-5 per concept, which is too small a sample for stable FA/DEF estimates even before adding the n=3-seed comparison on top. The real fix is not re-slicing the existing tiny per-concept sets differently, it's generating substantially more concept-specific instances (15-20+ per concept, via the same real-citation-grounded pipeline already used for the RGU expansion) before this comparison can be trusted either way. Until then, **OGDA's own-scenario claim should be treated as genuinely unresolved** — not supported by the diluted whole-benchmark measure, and not supported (or contradicted, cleanly) by the too-small concept-specific measure either.

**Bottom line**: the defensible current claim is narrower than originally hoped, arrived at by catching three separate statistical artifacts before they went in the paper (two n=1-vs-n=3 corrections, and now this concept-specific-vs-whole-benchmark disagreement). What survives across all this scrutiny: the collateral-suppression finding on rosiglitazone/Vioxx (§5 above, from the whole-benchmark n=3-vs-n=3 comparison, z=-3.07/-6.28) remains the most defensible and most thesis-relevant result — it wasn't re-tested by the concept-specific analysis (which only covered own-scenario, not collateral) and doesn't depend on the now-contested own-scenario FA/DEF numbers.
...
- **More concept-specific RGU instances for OGDA** (§5) — tried the concept-specific eval subset directly; at the 4-5 instances/concept that currently exist, it's too noisy to resolve the own-scenario question either way. Needs 15-20+ instances per concept, generated via the same real-citation pipeline already used for the RGU expansion, before this comparison is trustworthy.
